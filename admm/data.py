...
def get_cifar_data(num_clients: int = 100, batch_size: int = 20, seed: int = 42):
    (
        _,
        _,
        _,
...
def get_mnist_data(num_clients: int = 100, batch_size: int = 42, seed: int = 42):
...
def get_cifar100_data(num_clients: int = 100, batch_size: int = 20, seed: int = 42):
    (
        _,
        _,
        _,
    ...
    _, test_global_dl, _, _ = get_cifar100_dataloader(
    ...
    trainloaders = []
    for idx in range(num_clients):
        train_dl, _, _, _ = get_cifar100_dataloader(
            './data/cifar100', batch_size, 64, net_dataidx_map[idx]
        )
        trainloaders.append(train_dl)
    trainsets = [loader.dataset for loader in trainloaders]
    for i, dataset in enumerate(trainsets):
...
def mnist_partition_data(
    ...
    if iid:
        ...
        if power_law:
            ...
            # Two class per dataset
            #datasets = [
            #    ConcatDataset((tmp[idxs_list[2 * i]], tmp[idxs_list[2 * i + 1]]))
            #    for i in range(num_clients)
            #]
        # One class per dataset dataset 1 has label 1 and 2, dataset 2 has label 3 and 4, etc
        ...
        for i in range(num_clients):
            indices = []
            for label in range(i * labels_per_partition, (i + 1) * labels_per_partition):
...
def _power_law_split(
    ...
    num_partitions: int,
    num_labels_per_partition: int = 1,
    ...
    num_labels_per_partition: int
        ...
        a given partition will be long to the same two classes. default 1
